...
var base_numbers:Array = [-2,-1,1,2] ## base score values for the slices
...
func process_direction_input(direction:int)->void:
	...
	sound.stream = load("res://sounds/click1.wav")
	add_child(sound)
...
func process_confirm_input(direction:int)->void:
	...
	for x:Control in %covers.get_children():  # show the covers, increase the num selections, emit the signal, rotate
		if int(round(x.rotation_degrees)) == direction: 
			if x.visible: return 
			var rotatesound = AudioStreamPlayer2D.new()
			rotatesound.stream = load("res://sounds/click2.wav")
			x.visible = true
			num_selections += 1
			new_dir_chosen.emit(_current_value)
			rotate_slices()
			add_child(rotatesound)
			rotatesound.play()
